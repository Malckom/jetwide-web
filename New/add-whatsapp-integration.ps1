...
function Add-WhatsAppIntegration {
    ...
    if ($content -match '<link rel="stylesheet" href="[^"]*styles\.css"[^>]*/>') {
        ...
        if ($content -notmatch 'whatsapp\.css') {
            $newCssLink = "$stylesLine`r`n  <link rel=`"stylesheet`" href=`"${prefix}whatsapp.css`" />"
            $content = $content -replace [regex]::Escape($stylesLine), $newCssLink
        }
    }
    
    # Add JS script before </body> tag
    if ($content -match '</body>') {
        $scriptTag = "  <script src=`"${prefix}whatsapp.js`"></script>`r`n</body>"
...
Get-ChildItem -Path "$rootPath\pages\packages" -File -Filter "*.html" -ErrorAction SilentlyContinue | ForEach-Object {
    Add-WhatsAppIntegration -filePath $_.FullName
}

# Process New/pages folder
Write-Host "`n=== Processing New/pages folder ===" -ForegroundColor Yellow
Get-ChildItem -Path "$rootPath\New\pages" -File -Filter "*.html" -ErrorAction SilentlyContinue | ForEach-Object {
    Add-WhatsAppIntegration -filePath $_.FullName
}

# Process New/pages/packages folder
Write-Host "`n=== Processing New/pages/packages folder ===" -ForegroundColor Yellow
Get-ChildItem -Path "$rootPath\New\pages\packages" -File -Filter "*.html" -ErrorAction SilentlyContinue | ForEach-Object {
    Add-WhatsAppIntegration -filePath $_.FullName
}
...
Write-Host "  - whatsapp.js script before </body>" -ForegroundColor White
